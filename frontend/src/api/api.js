...
// API call to get client, events, and filterd events in one GET
export const getCombinedClients = async (id) => {
  try {
    const response = await apiClient.get(`/clients/combined/${id}`);
    return response.data;
  } catch (error) {
    throw error;
  }
...
// API call to get events, attendees, and services in one GET
export const getCombinedEvents = async (id) => {
  try {
    const response = await apiClient.get(`/events/combined/${id}`);
    return response.data;
  } catch (error) {
    throw error;
  }
