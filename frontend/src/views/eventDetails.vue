        ...
        <div class="px-10 py-20">
            ...
            >
                <div>
                    ...
                    <h3 class="italic">
                        Click table row to view client details
                    </h3>
                </div>
...
export default {
    ...
    async mounted() {
        try {
            // fetch combined data for a specific event id 
            const combinedResponse = await getCombinedEvents(this.$route.params.id);
            
            console.log(combinedResponse);

            // assign retrieve data to component properties 
            this.event = combinedResponse.event;
            this.clients = combinedResponse.clients;
            this.services = combinedResponse.services;
        ...
        async submitDeleteEvent() {
            try {
                ...
                if (this.event.attendees.length !== 0) {
                    toast.info(
                        "Event cannot be deleted since it has attendees."
                    );
                    return;
                }
