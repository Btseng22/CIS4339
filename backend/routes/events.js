...
// consolidate getEvendByID, getEventAttendees, and getServices into one GET
router.get('/combined/:id', authMiddleWare, async (req, res) => {
  try {
    const eventID = req.params.id;

    // Promise all to make concurrent API calls
    const [eventData, attendeeData, serviceData] = await Promise.all([
        
        // fetch event data
        events.findOne({ _id: eventID, org: org }),

        // Use async/await to wait for the result of the query
        (async () => {
          ...
          if (!event) {
            ...
            return clients.find({ _id: { $in: attendeeIds } }).exec(); // Use exec() here as well
          }
        })(),

        // fetch service data
        services.find({ org: org }).sort({ updatedAt: -1 })
